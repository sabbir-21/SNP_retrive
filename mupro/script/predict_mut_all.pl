...
#Modified by: Denis Bauer and Jianlin Cheng, 2/25/2006
###########################################################################################################################
...
$input = shift @ARGV;
$method = shift @ARGV; 
#iterates though all 20 AA
$loop=1;
#indicates if substituting AA was a *
$indicator=-1;
#summs up scores and devides the value with 20
$scoreOverAll=0;
#collects the AA which increases the score
#@increasingAA;
...
{
	#Set loop to 20 if AA was set to *
	if($rep_aa eq "*"){
		$loop=length($amino_acids);
		$indicator=1;
	}
	else{
		$error = "the substitute amino acid is not standard amino acid or *.";
	}
}
if ($error ne "")
{
...
while($loop>0){
	...
	print TMP_SEQ "0"; 
	#picks the for this loop current AA from $amino_acids
	if($indicator == 1){
		$rep_aa=substr($amino_acids,20-$loop,1);
	}
	#decreasing iteration variable
	$loop--;
	...
	if ($org_i == $rep_i)
	{
		if ($indicator == 1)
		{
			#skip the same amino acid
			next; 
		}
		else
		{
			die "original amino acid is the same as substitute amino acid. stop.\n";	
		}
	}
	...
	#output is normal if there is only one AA to evaluate
	if ($score > 0 && $indicator!=1)
	{
	...
	#fetches the values from this loop
	else{
		$scoreOverAll+=$score;
		if($score>0){
			@increasingAA=(@increasingAA, $rep_aa);
			@increasingEng = (@increasingEng, $score);
		}
		else
		{
			@decreasingAA=(@decreasingAA, $rep_aa);
			@decreasingEng = (@decreasingEng, $score);
		}
	}
	close RES; 
	`rm $input.svm.seq`; 
...
#output for evaluation of all AA
if($indicator==1){
	$numberIncreasing=@increasingAA;
	$numberDecreasing = @decreasingAA;
	print "$numberIncreasing mutations INCREASE the stability of the protein:\n";

	foreach ($i = 0; $i < $numberIncreasing; $i++)
	{
		print "AA = $increasingAA[$i]\tdelta G = $increasingEng[$i]\tINCREASE\n";
	}
	print "\n";
	print "$numberDecreasing mutations DECREASE the stability of the protein:\n";
	foreach ($i = 0; $i < $numberDecreasing; $i++)
	{
		print "AA = $decreasingAA[$i]\tdelta G = $decreasingEng[$i]\tDECREASE\n";
	}
}
